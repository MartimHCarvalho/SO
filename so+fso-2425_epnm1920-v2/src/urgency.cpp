...
int doctor_iteration(int id) // return value can be used to request termination
{
   ...
   if(is_closed_pfifo(&hd->doctor_queue))return 1;
   check_valid_patient(patient);
...
void patient_goto_urgency(int id)
{
   ...
   //mutex_init(&hd->all_patients[id].patient_mutex, NULL); -> resulta mas não é aconselhado
   //cond_init(&hd->all_patients[id].patient_is_done, NULL); -> resulta mas não é aconselhado
...
{  
   ...
   //cond_destroy(&hd->all_patients[id].patient_is_done);  resulta mas é perigoso
   //mutex_destroy(&hd->all_patients[id].patient_mutex);   resulta mas é perigoso
